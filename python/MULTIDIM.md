...
PyQUBO++ supports **multi-dimensional variables** and **multi-dimensional integer variables** of arbitrary depth using the functions `var()` and `var_int()`, respectively.
...
PyQUBO++ allows you to define **multi-dimensional expressions** with arbitrary depth using the function `expr()`:
...
Since PyQUBO++ vectors support Python iteration, nested for loops can be used:
```python
from pyqbpp import var
