---
layout: default
title: "PyQUBO++ Reference Variables"
---

# Quick Reference: Variables and Expressions (PyQUBO++)

## Data types in PyQUBO++
PyQUBO++ uses Python's native `int` type for coefficients, energy values, and constants.
Since Python integers have unlimited precision, there is no need to specify `coeff_t` or `energy_t` as in the C++ version.

## Printing objects
All PyQUBO++ objects can be printed using `print()` or converted to strings using `str()`:
```python
print(obj)
s = str(obj)
```
...
Usually, there is no need to use these properties explicitly in PyQUBO++ programs.
