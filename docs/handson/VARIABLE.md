...
```txt
f.simplify_as_binary() = 1 -a -b -c +a*b +a*c +b*c
```

Note that these member functions update the expression stored in `f`.
If you do not want to modify `f`, you can instead use the global functions
`qbpp::simplify(f)` and `qbpp::simplify_as_binary(f)`, which return the simplified expressions without changing `f`.
